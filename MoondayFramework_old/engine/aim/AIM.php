...
//require_once("http.php");  // Uncomment These Lines when Live
//require_once "CSVTokenizer.php"; // Uncomment These Lines when Live
...
class AIM {
    ...
    public function fetch() {
        ...
        echo "SendRequest succeeded\n";

        $body = "";
        while (
            (!($error = $http->ReadReplyBody($buffer, AIM::MAX_RESPONSE_SIZE)))
            && strlen($buffer) > 0) {
            echo "Read in " . strlen($buffer) . " response bytes\n";
            $body .= $buffer;
        }
        if ($error) {
            $http->Close();
        ...
        echo "ReadReplyBody succeeded (" . strlen($body) . " bytes total)\n";

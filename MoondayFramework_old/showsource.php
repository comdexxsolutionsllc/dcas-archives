...
<?php
$h = new PHP_Highlight;
$allowed = array('php', 'inc', 'class', 'inc.php', 'class.php', 'class.inc.php');
// Do not modify below:
function strip_ext($name) { $ext = strrchr($name, '.'); if($ext !== false) {$name = substr($name, 0, -strlen($ext));} return $name; }
$file = $_GET['page'];
$strip_string  = strip_ext($file); $search_string = end(explode('.',$file));
if(file_Exists($file)){ if(in_array($search_string, $allowed)){ $h->loadFile($_GET['page']); $contents = $h->toList(false); }else{echo 'You do not have access to '.$file.'!';}}else{echo "The file ($file) does not exist.";     }  
?>